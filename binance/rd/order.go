...
// rdOrders : readout order datas from binance.com, datas saved in order datas
func (ths *Reader) rdOrders() bool {
	...
	_L.Error("Binance : Client get(order) has error :\n%+v", err)
	ths.initOrderParams()
	return false
}
...
func (ths *Reader) decodeOrders(b []byte) (interface{}, error) {
	...
	if err != nil {
		_L.Error("Binance : decodeOrders has error :\n%+v", err)
		_L.Trace("Binance : decodeOrders orgdata [ %s ]", string(b))
	}
	return orders, err
}
