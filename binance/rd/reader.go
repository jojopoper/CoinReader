...
func (ths *Reader) baseInit() {
	...
	ths.Address = "binance.com"
	ths.currentVer = "v1"

	ths.OrderAddr = fmt.Sprintf("https://api.%s/api/v1/depth?symbol=%s%s",
		ths.Address, ths.Coin.Name, ths.Monetary.Name)

	ths.HistoryAddr = fmt.Sprintf("https://api.%s/api/v1/trades?symbol=%s%s",
		ths.Address, ths.Coin.Name, ths.Monetary.Name)
}
...
func (ths *Reader) initOrderParams() {
	...
	if err := ths.SetClient(ths.orderClt); err != nil {
		_L.Error("Binance : InitOrderParams set http client has error \n%+v", err)
	}
}
...
func (ths *Reader) initHistParams() {
	...
	if err := ths.SetClient(ths.historyClt); err != nil {
		_L.Error("Binance : initHistParams set http client has error \n%+v", err)
	}
}
